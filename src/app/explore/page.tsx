...
export default function ExploreRoomsPage() {
  ...
  const [userSearchState, setUserSearchState] = useState<'idle' | 'loading' | 'empty' | 'error'>('idle');
  ...
  async function handleUserSearch(event: React.FormEvent<HTMLFormElement>) {
    ...
    const username = normalizeUsername(userQuery);
    if (!username) return;
    ...
    try {
      ...
      if (!profile) {
        setUserSearchState('empty');
        return;
      }
      setUserResult(profile as UserSearchResult);
    ...
    <div className="omni-internal">
      ...
      <main className="omni-main-content explore-main">
        <section className="glass-card-ambient explore-control-card">
          ...
          <form onSubmit={(event) => void handleUserSearch(event)} className="explore-code-form">
            <input id="user-search" type="search" placeholder="Search by username..." value={userQuery} onChange={(event) => { setUserQuery(event.target.value); setUserResult(null); setUserSearchState('idle'); }} className="omni-input" />
            <button type="submit" disabled={!userQuery.trim() || userSearchState === 'loading'} className="omni-button omni-button-primary shrink-0">{userSearchState === 'loading' ? 'Searching...' : 'Search'}</button>
          </form>
          {userResult && <Link href={`/profile/${userResult.id}`} className="person-row explore-user-result"><span className="person-avatar">{userResult.avatar_url ? <img src={userResult.avatar_url} alt="" /> : (userResult.display_name || userResult.username || 'O').slice(0, 1).toUpperCase()}</span><span><strong>{userResult.display_name || userResult.username || 'OmniLume member'}</strong><small>@{userResult.username || 'member'} {userResult.is_private ? '· Private profile' : '· Public profile'}</small></span><OmniIcon name="arrow" size={15} /></Link>}
          {userSearchState === 'empty' && <p className="mt-3 text-sm text-neutral-500">No users found.</p>}
          {userSearchState === 'error' && <p className="form-error mt-3" role="alert">Enter a valid username to search.</p>}
        </section>
