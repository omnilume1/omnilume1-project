...
type MemberAction =
  ...
  | "member"
  | "transfer";
const memberIdentityLabels = new Map<string, string>();
...
export default function MembersTab({
  ...
  const { onlineUserIds, currentUserId, roomControlVersion } =
    useRoomRealtime();
  ...
  const fetchMembers = useCallback(async () => {
    setLoading(true);
    try {
      const [memberData, controlStateData, identityData] = await Promise.all([
        getRoomMembersList(roomId),
        getRoomMemberControlStates(roomId),
        getRoomMemberIdentities(roomId),
      ]);
      setMembers((memberData as RoomMember[]) || []);
      setMemberStates((controlStateData as MemberState[]) || []);
      setIdentities((identityData as MemberIdentity[]) || []);
      (identityData as MemberIdentity[]).forEach((identity) =>
        memberIdentityLabels.set(
          identity.user_id,
          identity.room_display_name ||
            identity.account_display_name ||
            identity.username ||
            `User ${identity.user_id.slice(0, 8)}`,
        ),
      );
      setError(null);
    } catch (loadError) {
  ...
  const identityByUserId = useMemo(() => new Map(identities.map((identity) => [identity.user_id, identity])), [identities]);
  const approvedMembers = members.filter(
    (member) => member.join_status === "approved",
  );
  const pendingMembers = members.filter(
    (member) => member.join_status === "pending",
  );
  const restrictedFormerMembers = memberStates.filter(
    (state) =>
      !members.some((member) => member.user_id === state.user_id) &&
      (state.restriction_types?.length ?? 0) > 0,
  );

  if (loading)
    return (
      <div className="omni-state-screen min-h-0 p-4 text-xs text-neutral-500">
        Loading members...
      </div>
    );
    ...
    <div className="chat-panel flex h-full flex-col">
      ...
      <div className="chat-scroller flex flex-col gap-6 custom-scrollbar">
        {error && (
          <p
            role="alert"
            className="rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-xs text-red-200"
          >
            {error}
          </p>
        )}
        {canManage && pendingMembers.length > 0 && (
          <section>
            <h4 className="mb-3 text-[10px] font-bold uppercase tracking-wider text-amber-500">
            ...
            <div className="flex flex-col gap-3">
              {pendingMembers.map((member) => (
                <div
                  key={member.user_id}
                  className="flex flex-col gap-3 rounded-lg border border-amber-900/30 bg-[#121212] p-3"
                >
                  <span
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        void handleManage(member.user_id, "approve")
                      }
                      className="flex-1 rounded bg-emerald-500/10 py-1.5 text-xs font-semibold text-emerald-500 hover:bg-emerald-500/20"
                    >
                      Approve
                    </button>
                    <button
                      type="button"
                      onClick={() =>
                        void handleManage(member.user_id, "reject")
                      }
                      className="flex-1 rounded bg-red-500/10 py-1.5 text-xs font-semibold text-red-500 hover:bg-red-500/20"
                    >
                      Reject
                    </button>
                  </div>
        ...
        <section>
          <h4 className="mb-3 text-[10px] font-bold uppercase tracking-wider text-neutral-500">
            In room ({approvedMembers.length})
          </h4>
          <div className="flex flex-col gap-2">
            {approvedMembers.map((member) => {
              ...
              return (
                <article
                  key={member.user_id}
                  className="relative flex items-center justify-between gap-2 rounded-lg border border-transparent p-3 transition hover:border-neutral-800 hover:bg-neutral-900/50"
                >
                    ...
                    <div className="min-w-0">
                      ...
                      {member.role === "guest" && (
                        <span className="mt-1 block text-[10px] text-amber-300">
                          ...
                          {formatExpiry(memberState?.guest_expires_at ?? null)}
                        </span>
                      )}
                      {(memberState?.restriction_types?.length ?? 0) > 0 && (
                        <span className="mt-1 block text-[10px] text-red-300">
                          Restricted:{" "}
                          {memberState?.restriction_types?.join(", ")}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        {canManage && restrictedFormerMembers.length > 0 && (
          <section>
            <h4 className="mb-3 text-[10px] font-bold uppercase tracking-wider text-red-300">
              Restricted accounts
            </h4>
            <div className="space-y-2">
              {restrictedFormerMembers.map((state) => (
                <article
                  key={state.user_id}
                  className="rounded-xl border border-red-400/20 bg-red-500/5 p-3"
                >
                  <p className="text-xs font-semibold text-white">
                    {userLabel(state.user_id)}
                  </p>
                  <p className="mt-1 text-xs text-red-200">
                    {state.restriction_types?.join(", ")}
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {state.restriction_types?.includes("ban") && (
                      <button
                        type="button"
                        onClick={() =>
                          setConfirmAction({
                        ...
                        className="omni-button omni-button-ghost text-xs"
                      >
                        Remove ban
                      </button>
                    )}
                    {state.restriction_types?.includes("block") && (
                      <button
                        type="button"
                        onClick={() =>
                          setConfirmAction({
      ...
      {confirmAction && (
        <div
          className="border-t border-white/10 bg-[#121319] p-4"
          role="alertdialog"
          aria-label="Confirm member action"
        >
          <p className="text-sm font-semibold text-white">
            {confirmAction.action === "transfer"
              ? "Transfer room ownership?"
              : `${confirmAction.action[0].toUpperCase()}${confirmAction.action.slice(1)} this member?`}
          </p>
          <p className="mt-1 text-xs leading-5 text-neutral-400">
            {confirmAction.action === "transfer"
              ? "You will become an admin and must receive ownership again to reverse this."
              : "This action is checked and enforced by the room backend."}
          </p>
...
function MenuAction({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-3 py-2 text-left text-xs font-medium ${toneClass}`}
    >
      {label}
    </button>
  );
