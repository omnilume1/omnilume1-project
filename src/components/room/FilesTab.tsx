...
import OmniFileUpload from '@/components/ui/OmniFileUpload';
...
export default function FilesTab({ roomId, currentUserRole }: FilesTabProps) {
  ...
  const handleFilesSelected = async (files: File[]) => {
    for (const file of files) await uploadSelectedFile(file);
  };
  ...
  return (
    <div className="chat-panel flex h-full flex-col border-l border-white/10">
      ...
      <div className="message-composer">
        ...
        ) : <OmniFileUpload onFilesSelected={(files) => void handleFilesSelected(files)} disabled={!canCast} accept="video/*,audio/*" label={canCast ? 'Upload media (under 50MB)' : 'Owner/admin upload only'} helper="Drop audio or video files - up to 50MB each" />}
      </div>
    </div>
  );
}
